    ...
    with open(outCyntenatorGenome, 'w') as f_genome:
        ...
        for chr in genomeRewritten:
            for (idxG, (gn, gs, fn)) in enumerate(genomeRewritten[chr]):
                ...
                print("%s %s %s %s %s" % (fn, chr, str(idxG), str(idxG+1), ("+" if gs == 1 else "-")), file=f_genome)
    ...
    for i,l in enumerate(open(outCyntenatorAlignment, 'r').readlines()):
        if i == 0:
            ...
            if s1 == '.' or s2 == '.':
                ...
                if fam1n != fam2n:
                    print('mismatch in idSb = %s : paired g1fn %s != paired g2fn %s' % (idSb, fam1n, fam2n), file=sys.stderr)
                    # ex :
...
def launchCyntenator(genome1, genome2, families,
    ...
    genes1ToRemove = set([g.n for (chr, chrom) in genome1.items() for (gIdx, g) in enumerate(chrom) if gIdx not in tb2g1[chr].old])
    genes2ToRemove = set([g.n for (chr, chrom) in genome2.items() for (gIdx, g) in enumerate(chrom) if gIdx not in tb2g2[chr].old])
    ...
    print(bashComand, file=sys.stderr)
...
if __name__ == '__main__':
    ...
    print(alignmentAsLightGenome, file=sys.stdout)
